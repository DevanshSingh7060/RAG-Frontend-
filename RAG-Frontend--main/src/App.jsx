...
export default function App() {
  ...
  useEffect(() => {
    if (!playIntro) {
      const timer = setTimeout(() => setPulseGlow(false), 400); // 350-450ms glow pulse v2.4
  ...
  useEffect(() => {
    ...
    // Timer 2: welcome -> transitioning (after 2600ms)
    const t2 = setTimeout(() => {
      setIntroStep('transitioning');
      setIntroStarCoords({
        top: isMobile ? '28px' : '36px',
        left: isMobile ? '32px' : '40px',
        size: 82, // Keep size 82px to maintain SVG scale transition
        opacity: 1,
        scale: 28 / 82 // Scale down to 28px
      });
    }, 2600);

    // Timer 3: transitioning -> settled (after 3200ms)
    const t3 = setTimeout(() => {
  ...
  // Sequenced streaming
  const streamAIResponse = async (userPrompt) => {
  ...
  const handleSelectModel = (modelId) => {
    ...
    // Step 1: Smooth fade to dark reset (180ms)
    setIsDarkReset(true);
    setIsModelSwitching(false);
    setIsFullGlowActive(false);
    setIsTracerFading(false);

    setTimeout(() => {
      // Step 2: Start Tracing Phase (2.0s duration)
      setIsDarkReset(false);
      setIsModelSwitching(true);

      setTimeout(() => {
        // Step 3: Tracing complete, trigger Full Glow & pulse (220ms duration)
        setIsFullGlowActive(true);

        setTimeout(() => {
          // Step 4: Fade Out (700ms duration)
          setIsTracerFading(true);

          setTimeout(() => {
            // Step 5: Clean up and restore normal backgrounds
            setIsModelSwitching(false);
            setIsFullGlowActive(false);
            setIsTracerFading(false);
          }, 700); // 700ms fade out
        }, 220); // 220ms full glow & pulse
      }, 2000); // 2.0s tracing duration
    }, 180); // 180ms dark reset duration
  };
      ...
      {playIntro && introStep !== 'settled' && introStep !== 'void' && (
        ...
        >
          <div className={(introStep === 'welcome' || introStep === 'transitioning') ? 'star-welcome-scale' : ''}>
            <div className={(introStep === 'welcome' || introStep === 'transitioning') ? 'star-welcome-rotate' : ''}>
            ...
            >
              <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', flex: 1, gap: '12px' }}>
                ...
                <input
                  ...
                  style={{ ...styles.heroInput, width: '100%' }}
